...
La majorité des types génériques seront déjà renseignées. Dans certain cas, vous devrez les rentrer manuellement (plugin virtuel par exemple) 
...
*ATTENTION* Chaque équipement doit disposer d'au moins un type générique de type "info". S'il n'y a pas d'info, l'équipement ne sera pas disponible sur Homebridge.
...
[IMPORTANT]
(réservé à un public averti). ATTENTION : Il n'y aura aucun support pour cette partie
